...
#im = cv2.imread('D:\\lalalalala\\documents\\beautifulLife\\1.jpg')
cap = cv2.VideoCapture('D:\\lalalalala\\documents\\beautifulLife\\TestSamples\\videos\\AvaerageFilter_ForeGround.mp4')
while(cap.isOpened()):
    # get a frame
    ret, frame = cap.read()
    imgray = cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)#参数是原图和转色类型，输出是灰度图
...
#for c in contours:
    for i in range(1,len(contours)):
        ...
        if M["m00"] != 0:
            cX = int(M["m10"] / M["m00"]) #质心坐标。m00表示对象的（二进制）面积或（灰度图）灰度等级。
            cY = int(M["m01"] / M["m00"]) #m10, m01表示原始图像矩。
        else:
            cX, cY = 0, 0
        cv2.circle(drawing, (cX, cY), 2, (255, 255, 255), -1)
